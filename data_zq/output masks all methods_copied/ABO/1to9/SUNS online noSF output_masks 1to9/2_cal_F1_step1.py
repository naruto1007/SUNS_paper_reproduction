...
def Performance_Cal(root_dir, result_dir, data_type_1, data_type_2, data_type_3, list_Exp_ID, list_Exp_ID_test, method_type):
    ...
    for CV in list_CV:
        ...
        for CV_test in list_CV:
            if CV_test != CV:
                ...
                # Mask_sparse
                filename_output = dir_OutputMasks + Exp_ID + '_' + Exp_ID_test + '_sparse.mat'
                data_Masks = loadmat(filename_output)
                Masks_2 = data_Masks['Masks'].transpose()
                # calculate
                (Recall_test[CV_test], Precision_test[CV_test], F1_test[CV_test]) = GetPerformance_Jaccard_2(GTMasks_2, Masks_2, ThreshJ=0.5)
...
data_type_2 = "275"
data_type_3 = "1to9"
method_type = "SUNS online noSF output_masks 1to9"
