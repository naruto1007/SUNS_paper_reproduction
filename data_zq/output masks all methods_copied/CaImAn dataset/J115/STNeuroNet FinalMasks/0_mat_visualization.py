...
def mat_visualization_output(filename):
    try:
        mat = h5py.File(filename, 'r')
        Masks = np.array(mat['finalSegments']).transpose([1, 0, 2])
        mat.close()
    except OSError:
        mat = loadmat(filename)
        Masks = np.array(mat["finalSegments"]).transpose([1, 0, 2])
...
dir_Masks_GT = "D:\PyCharm_project\SUNS_paper_reproduction\data_zq\CaImAn dataset\J115\GT Masks"
filename_GT = os.path.join(dir_Masks_GT, "FinalMasks_J115_part11.mat")
...
filename_output = "FinalSegments_J115_part11.mat"
